...
class MyFigure:
    ...
    def setup(self): 
        self.ax_index, self.axes, self.fig  = 0, [], matplotlib.pyplot.gcf() 
        self.rows, self.cols, self.WD, self.HT = 20, 33, 7.1, 6.5

    def create(self): 
        self.letters, pred_pairs, idx, cs1 = ['a','b','c','d'], [], 0, 8 
        for rl in [0,10]:
            for cl in [0,18]: 
                try: 
                    ...
                    if 'pred' in T.pts: 
                        ...
                        pred_pairs.append([T, self.axes[-4::], self.letters[idx-1]]) 

                except: pass 
        self.fig.set_size_inches(self.WD, self.HT) 
        for T,axes,nx in pred_pairs:
            
            self.progress.set_panel(nx) 
            dp = DP.PredPlot(self) 
            dp.draw_odds(T, axes, QT=True) 
        try: 
            ax = self.axes[len(self.axes)-5] 
            axl = DV.AxLims(ax) 
            x1, x2, y1 = axl.xMin + 1.25*axl.xStep, axl.xMin + axl.xStep*12, axl.yMin - 5.5*axl.yStep 
            axl.ax.plot([x1,x1+15],[y1,y1], clip_on=False,color='blue',lw=1) 
            axl.ax.scatter(x1,y1, color='blue', s=28,clip_on=False) 
            axl.ax.scatter(x1+15,y1, color='blue', s=28,clip_on=False) 
            ...
            DV.draw_square(axl.ax, x1-axl.xHop*5, x2+axl.xStep*10, y1-0.3, y1 + axl.yStep*1.6) 
        except: pass 
        return 

    def finish(self): 
        plt.subplots_adjust(left=0.04, bottom=0.01, right=0.99, top=0.96,wspace=0.05, hspace=0.05) 
        for i,idx in enumerate([0,4,8,12]): self.axes[idx].set_title('$'+self.letters[i]+'$', x=-0.05,y=0.96,fontsize=15) 
        self.progress.save()
        return
