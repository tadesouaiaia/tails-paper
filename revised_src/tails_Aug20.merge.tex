...
and at least one significant rare variant, 49 had reduced POPout effect (Binomial test $P=6.5\times 10^{-5}$), 
...
\subsubsection*{UK Biobank genotye data} 
...
Genetic QC in All of Us was conducted using standard filters, as used
in the UKB.  SNPs with MAF $<$ 0.01, genotype missingness > 0.02, or
Hardy-Weinberg equilibrium test $P < 10^{-8}$ were excluded. The same
greedy algorithm used to maximise sample retention of unrelated
individuals as used in the UKB, was applied here. Using the
higher-powered publicly available European ancestry GWAS summary
statistics generated from the entire UKB \cite{neale} to provide SNP
weights, trait PRS were calculated \cite{prsice} using the same
$P$-value threshold relevant to each trait and trained in the UKB
European ancestry analysis (see above).
...
\vspace*{0.3cm} 
\subsection*{The POPout test}} \label{sec:methods:popout} 
...
\[
\text{POPout effect} = \frac{1}{n}\sum_i^n |{f(y_i)}| - |{PRS_i}|\quad
                         {i\in{1,\dots,n} \ : \ y_i >K}
\]
where $K=\Phi^{-1}(0.99)$ (testing the top 1\% tail), $\Phi^{-1}$ is
the inverse-normal cumulative density function, $f(y)$ is the
predicted PRS from the regression given trait value $y$ and $n$ is the
number of observations in the quantile.

\subsubsection*{POPout test QC}
...
\subsubsection*{Replication of POPout associations} 
We replicated our primary POPout analyses in three different
independent cohorts: \textbf{(1)} UKB European ancestry individuals
with repeat measures (baseline and follow-up at $\sim$5 years),
\textbf{(2)} Non-European Multi-Ancestry UKB individuals, \textbf{(3)}
the European ancestry samples in the US-based All of Us biobank. For
each cohort, traits with at least 5000 samples that passed our POPout
QC, were included and tested for matched discovery of POPout effects
and correlation as described above.

\subsubsection*{Estimating power of POPout on replication analyses}
In our primary analysis, we set a minimum total sample size threshold
of 100k (i.e. at least 50k target samples). To explore the power of
{\color{green}
POPout at different sample sizes, we applied POPout to subsets ranging
from 1k to 50k target samples, with subset sizes incremented by 5k.
Analyses were repeated 100 times for each sample size. Results were
compared with the primary analysis with respect to \textbf{(1)}
the number of FDR 5\% significant tails replicated at $P<0.05$ and 
\textbf{(2)} the average correlation (Pearson) across POPout effects 
between the primary and subset analysis. As shown in \sref{sup_rep}, 
at subset sizes of 5,000 the average correlation is 0.715 
(all 100 repetitons are statistically significant $P<0.05$).  This 
threshold was therefore used as a minimum in the replication analyses. 
}
}


%significant at FDR 5\%; replication power was defined as the
%proportion of sampled subsets with P $<0.05$ for these associations.
%\sref{sup_rep} shows that, for a sample size of 5,000, we achieved x\%
%power to replicate associations with an error rate of y. This
%threshold was therefore used in the replication analyses.
%}
...
\subsubsection*{Calculating association between POPout effects and rare exome hits}
Backman et al.\cite{Backman2021} provide association results from the
UK Biobank exome sequencing study across 3,994 traits. These results
include 65 of the 74 traits analysed in our target set.  For each of
these 65 traits, in each tail, we counted the number of genes with at
least one significant association. Following Backman et
al.\cite{Backman2021}, we defined gene significance as either a
single-variant deleterious, missense, or predicted loss-of-function
association, or a gene-burden association, at P
$\le 2.18\times 10^{-11}$. Across the 130 trait tails, we calculated
the Pearson correlation between the number of genes with significant
exome associations and the POPout effect size.

\subsubsection*{Incorporating WGS data to identify significant rare variants}
The UK Biobank recently released whole-genome sequencing (WGS) data
for approximately 500,000 participants \cite{ukb, ukb3}. This dataset
has an average coverage depth of 23.5$\times$, enabling reliable
detection of rare genetic variants typically absent from PRS analyses.
We analysed the previously QCed European-ancestry samples using the same
base samples and standardised phenotypes across our 74 traits. GWAS (with standard QC filters, variant missingness <$<0.02$) 
followed by clumping and thresholding ($r^2=0.1$, window=250kb) was
then applied using PLINK 1.9 to identify independent associations
\cite{plink, bt_10}.  In addition to the covariates used in the common
variant GWAS, rare variant principal components (PCs) and the top 40
components were included as covariates in the GWASs.

For each trait two rare variant PRS were constructed using clumped
variants with P $\le 2.18\times 10^{-11}$ \cite{Backman2021}; one using variants with
0.001 $\le$ MAF $<$ 0.01 and the other with variants with 0.0001 $le$
MAF $<$ 0.001. To assess the effect of rare variants on POPout, POPout was
recalculated using the combined-variant PRS (the rare variant PRS plus the
common-variant PRS). PRS recovery was then calcaulated as
\[
  \mbox{POPout rare variant recovery}=\frac{{POPout}_{common}-POPout_{combined}}{POPout_{common}}
\]

\subsection*{Heritability attributable to genetic effects not included in
  the PRS}
The observed POPout effect can be approximated by the expected minus
the observed PRS for individuals in the top (or bottom) 1\% of the
trait distribution
\begin{align}
  \mbox{POPout effect}
  &= \frac{1}{n}\sum_i^n \left(
    \left\lvert{f(y_i)}\right\rvert -
    \left\lvert\mbox{PRS}_i \right\rvert \right)\quad
    {i\in{1,\dots,n} \ : \ y_i >K}. \nonumber \\
  &\approx \left\lvert \E(\mbox{PRS} \mid y>K) \right\rvert -
    \left\lvert \overline{\mbox{PRS}} \mid (y>K) \right\rvert
  \label{popout}
%  - \frac{1}{n_K}\sum_{i:y_i<K} \mbox{PRS}_i  \\
\end{align}
where $\overline{\mbox{PRS}} \mid (y>K)$ is the empirical conditional
mean of the PRS. The following explanation is for the upper tail,
soliution for the lower follows similarly. We assume a proportion
$\theta$ of individuals are in the top 1\% of the trait distribution
due to large variants of rare effect that are not captured by the
PRS. Given $\theta$, the observed mean PRS in the tail can be written
as
\begin{align*}
  \overline{\mbox{PRS}} \mid (y>K) &=
...
genetic variants and independent Gaussian environmental effects i.e. without
unobserved variants of large effect. We make the simplifying assumumption
$y^\prime\sim\N(0,1)$. If all variants of large effect (omitted by the
PRS) have effect size $b$, when an individual carries such a
...
\intertext{Substituting into (\ref{popout}) and rearranging gives}
  \theta &= \frac{\mbox{POPout effect}}{\E(\mbox{PRS}\mid y^\prime>K) - E(\mbox{PRS}\mid y^\prime>K-b)}
\end{align*}
Since the trait $y^\prime$ and the PRS are standardised to have
variance of 1, the variance explained by the PRS is $r^2$ and the
environmental effect $e$ is Gaussian
\begin{align*}
  y^\prime &= rPRS + e
  \intertext{Since the correlation between the PRS and $y^\prime$ is
             $r$, reversing the regression gives}
  \mbox{PRS} &= ry^\prime + e^\prime
  \intertext{where $e^\prime$ is a zero mean error term. Thus the conditional expectation of the PRS is}
  \E(\mbox{PRS}\mid y^\prime>K) &= r\E( y^\prime \mid y^\prime>K)\\
  &= r\frac{\phi(K)}{1-\Phi(K)}                               
\end{align*}
where $\phi$ and $\Phi$ are the normal density function and its
cumulative density function. Therefore, we can calculate the
proportion of individuals in the tail due to variants of large effect,
$\theta$, from the POPout effect.

We next calculate the required frequency $\pi$ of rare variants of
large effect $b$ to put a proportion $\theta$ in the tail. The
proportion of individuals in the population in the tail with a rare
variant of large effect is $\pi(1-\Phi(K-b))$. Similarly, the proportion
in the tail without a large effect rare variant is
$(1-\pi)(1-\Phi(K))$. Therefore, the proportion $\theta$ in the tail with
a large effect rare variant can be written as
\begin{align*}
  \theta &= \frac{\pi(1-\Phi(K-b))}{ \pi(1-\Phi(K-b)) + (1-\pi)(1-\Phi(K)) }
  ...
  m &= \frac{\theta\Phi(K)}{2\pi^\prime\left(\theta\Phi(K) + (1-\theta)\Phi(K-b)\right) }
    ...
    \V(\mbox{rare effects}) &= \sum_{i=1}^2 m_i 2 b^2 \pi^\prime
                              (1-\pi^\prime) \\
  &\approx b^2\sum_{i=1}^2 \pi_i \quad \mbox{for small $\pi^\prime$}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection*{Forward-in-time simulation} \label{sec:methods:sims} 
...
We ran this simulation for 100 independent replicates and reported
data in each simulation by logging, at each generation, the phenotypic
mean and standard deviation of the population, as well as statistics
regarding the mutations present in the populations, including allele
frequencies and effect sizes, and the mutational profile of each
individual.  To analyse the enrichment of rare variant architecture at
different quantiles, the population was stratified across 100 centiles
(each including 100 individuals) and the number of rare and common
alleles carried by individuals in each centile was calculated under
neutrality vs under selection.  For comparison purposes, "weak
selection" corresponded to simulations performed using a multiplier of
$k=1$ in equation 3, while "strong selection" corresponded to
simulations with $k=100$. "Large effects" were considered those in the
top 10\% of effect sizes genome-wide.  The mean of the log-transformed
odds ratio across all 100 independent simulation replicates was used
as a measure of enrichment and a $t$-test was performed to test for
significant differences from zero.  To analyse the impact that
low-frequency variants have on prediction and to mimic the calculation
of common variant PRS in real data, a PRS was calculated using only
variants with \emph{MAF} $>1\%$ and compared to the true trait value
across the individuals in each centile.  The mean difference (POPout
effect) and 95\% confidence intervals in each centile were calculated
across the 100 independent simulation replicates. These simulations
reflect a subset of those performed in a recent simulation study that
we conducted \cite{Ori2024}.
...
\subsubsection*{Calculating association between POPout effects and lifetime
  reproductive success inferred selection}
To test the association between POPout effects and the selection
acting on corresponding traits as inferred by the lifetime
reproductive success model, we stratified traits into four categories
of selection. The first two of the four categories reflect
non-directional selection: \textbf{(1)} no selection
($\beta=0,\gamma=0$), in which POPout effects are not expected in
either tail, \textbf{(2)} Non-directional stabilising selection
($\beta=0,\gamma<0$), in which we hypothesise POPout effects in both
tails of equal magnitude. The third and fourth categories reflect
directional selection: \textbf{(3)} positive selection ($\beta>0$),
which can be linear ($\gamma=0$), disruptive (($\gamma>0$) or
stabilising ($\gamma<0$), in which we hypothesise greater POPout
effects in the lower tail vs the upper tail, \textbf{(4)} negative
selection ($\beta>0$; linear, disruptive or stabilising as for
positive), in which we hypothesise greater POPout effects in the upper
tail vs the lower tail. For each category, a t-test is applied to test
for the difference in mean POPout effects (excluding non-significant
POPout effects) between the lower and upper tails across the traits.

\subsubsection*{Calculating association between POPout effects and broad measures of fitness}
Five broad measures of fitness were considered: \textbf{(1)} Male and
\textbf{(2)} female fecundity (Field IDs: 2405 and 2734), \textbf{(3)}
self-reported, non-cancer illnesses (Field ID: 2002), \textbf{(4)}
total miscarriages and still births (Field IDs: 39290 and 3839),
\textbf{(5)} paternal age (calculated as the difference between
father's age (Field ID: 2946) and participant age (Field ID:
21022). Associations between POPout effects and these measures were
tested as follows. For each measure, the mean value was calculated
separately in the upper and lower (1\%) tail for every trait. Then,
the association between between these tail measure means and
corresponding POPout effect sizes was calculated as a Spearman's rank
correlation across the traits.
...
\subsection*{Evaluating impact of POPout effects on PRS prediction} 
To evaluate how observed POPout effects impact on the performance of
PRS, prediction, we emulate a scenario in which the quantitative trait
underlies a binary outcome (e.g. as is the case for BMI underlying
clinical obesity). We consider individuals above a specified trait
threshold value to be cases and individuals below the threshold to be
controls. We investigate thresholds at the 75\% and the 99\% points of
the trait distribution. For each, we use the PRS to predict
case/control status, and then generate a quantile plot, in which the
odds ratio of the given quantile is estimated in relation to the
central quantile via logistic regression
(\dref{fig2}{c}). Additionally, for each trait with $PRS$ $r^2$, we
simulated a quantitative phenotype with trait values given by
$T=r{PRS}+E$, where $PRS$ has a standard normal distribution and $E$
represents all other contributions to trait variance and is drawn from
a normal distribution with mean 0 and variance $(1-r^2)$. This
produces a simulated trait with a $\mathcal{N}(0,1)$ distribution and
$PRS$ $r^2$. Using the same number of target samples used in the real
data analysis for each trait, this simulation allows us to estimate
the expected predictive performance of the PRS based on the
corresponding $r^2$ and assuming uniform polygenicity throughout the
trait distribution.  The results of this analysis for all our index
traits are shown in \sref{sup_pred}.

\subsection*{Rare variant annotation}
Out of a total of 610 rare variants identified above, 143 were shared
across multiple traits and 67 were previously idenfied and had an
annotation in the CLINVAR database.  These SNPs and the phenotypes
they have been previously associated with are listed in
(\stref{sup_clinVar1}).

\subsection*{Alternative POPout tail quantiles} 
While we focus on POPout on the upper and lower 1\% of the trait
distribution, this value is arbitary and can be specified by the user
in our POPout software ({\tt PRStools.org}). \sref{sup_t1} shows
POPout results for tail quantiles of 0.1\%, 0.5\%, 5\%,
and 10\%. \sref{sup_t2} compares POPout rare+common variant recovery results
in the 1\% and 0.1\% tails.

\subsection*{Comparison of lifetime reproductive success results to Sanjak et al.}
\sref{sup_vish} compares results of our analyses of the lifetime
reproductive success data with those of Sanjak et
al. \cite{Sanjak2018}. Results are shown for Sanjak et al. sex
stratified results and our pooled results. Our analyses also differed
from Sanjak et al. by model selection criteria, we selected the best
fitting model by BIC whereas Sanjak et al. used P-values.  }

\subsection*{Sensitivity of lifetime reproductive success analyses to BIC Model choice}
For each pair of best and second best models as ranked by BIC we
performed the following comparison.  When the second best model was a
nested subset of the best model we calculated the likelihood ratio
test to determine if it was significantly better (P $<0.05$).  When
the second best model had more parameters we determiend that a
difference in BIC$<2$ was weak evidence of model
superiority \cite{bayesguys}. These two criteria identified 10 and 7
traits, respectively, for which there was justification to use the
alternate model.  This traits are shown in \stref{sup_bic} where we
additionally (\stref{sup_bic}{c} replace the top model with the
alternate model for these 17 traits and reestimate the relationship
between LRS and POPout effect observed in \fref{fig5}{d} - we observe
that the expected relatipnship persists.
...
Extended analysis from \fref{fig1}{c}, using PRS to predict trait values in the lower/upper 25\% and 1\%  
of the distribution.  
...
\textbf{b,} Across 43 traits with significant common-variant PRS POPOut and at least one significnat rare variant, the average POPout 
effect is greater when calculated at 0.1\% than 1\% (mean 0.51 vs 0.20, $P=6\time10^-16$). 
\textbf{c,} Across the same traits, POPout recovery is also greater at 0.1\% than 1\% (mean 0.17 vs 0.064, $P=0.005$). 
\textbf{d,} Mechanism behind recovery of tail signal from rare+common PRS is varied.  In some cases (Upper Tail: RBC Dist Width) rare variants uniformly 
...
\FloatBarrier \begin{figure}[!h] 
\includegraphics[trim=0cm 0cm 0cm 0cm, clip, width=0.75\linewidth]{Figures/sup_anil1.lowercase.pdf} 
...
\FloatBarrier \begin{figure}[!h] 
\includegraphics[trim=0cm 0cm 0cm 0cm, clip, width=0.75\linewidth]{Figures/sup_anil2.lowercase.pdf} 
...
\FloatBarrier \begin{figure}[!h] 
\includegraphics[trim=0cm 0cm 0cm 0cm, clip, width=\linewidth]{Figures/sup_anil3.lowercase.pdf} 
...
\FloatBarrier \begin{figure}[!h] 
\includegraphics[trim=0cm 0cm 0cm 0cm, clip, width=\linewidth]{Figures/bictable3.pdf} 
\caption{{\bf Model Selection: Sensitivity Analysis}
\textbf{a,} For 17 of 74 traits, the top model selected using BIC was not significantly better than 
the alternative (next best) model. 
\textbf{b,} The relationship between models of selection and POPout (top models). 
\textbf{c,} The relationship between models of selection and POPout using 17 alternative models of selection. 
...
\FloatBarrier \begin{figure}[!h] 
\includegraphics[trim=0cm 0cm 0cm 0cm, clip, width=\linewidth]{Figures/rareSupTable1.pdf} 
\caption{{\bf Rare+Common Variant PRS: Summary}
For 43 or 74 traits with one or more rare variants and FDR significant POPout present in the common PRS analysis, we 
compare the $R^2$, POPout effects/recovery, and tails odds for prediction of 1\% phenotype from 1\% PRS values 
for common and common+rare PRS models. 
} \label{sup_rareTable} \end{figure}
\FloatBarrier


\FloatBarrier \begin{figure}[!h] 
\includegraphics[trim=0cm 0cm 0cm 0cm, clip, width=\linewidth]{Figures/cliveSupTable1.pdf} 
\vspace*{-2cm} 
\caption{{\bf Estimating Unaccounted $h^2$} 
For 43 or 74 traits with one or more rare variants and FDR significant POPout present in the common PRS analysis, we 
estimate the $h^2$ unexplained from the PRS variants for the common PRS and for the rare+common PRS. 
} \label{sup_cliveTable} \end{figure}
\FloatBarrier


\setcounter{figure}{4}
\FloatBarrier \begin{figure}[!h] 
\includegraphics[trim=0cm 0cm 0cm 0cm, clip, width=\linewidth]{Figures/SnpTable1.pdf} 
\caption{{\bf SNP Annotation Table} 
Part I: CLINVAR SNP Annotation is present for many rare variants. 
} \label{sup_clinVar1} \end{figure}
\FloatBarrier

\setcounter{figure}{4}
\FloatBarrier \begin{figure}[!h] 
\vspace*{-1cm} 
\includegraphics[trim=0cm 0cm 0cm 0cm, clip, width=\linewidth]{Figures/SnpTable2.pdf} 
\caption{{\bf SNP Annotation Table} 
Part II: CLINVAR SNP Annotation is present for many rare variants. 
} \label{sup_clinVar1} \end{figure}
\FloatBarrier
...
\caption{{\textbf{Traits 1-25: Common Variant POPout Associated Results}}} \label{bigsup1} \end{figure} \FloatBarrier
...
\caption{{\textbf{Traits 25-50: Common Variant POPout Associated Results}}} \label{bigsup2} \end{figure} \FloatBarrier
...
\caption{{\textbf{Traits 50-74: Common Variant POPout Associated Results}}} \label{bigsup3} \end{figure} \FloatBarrier
