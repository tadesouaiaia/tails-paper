...
power to replicate the "gold standard" associations with an error rate
of y. This threshold was therefore used in the replication analyses.
...
exome associations and the POPout effect size (\dref{fig4}{b}).
...
all analysed traits.  Then, after applying standard QC filters (sample and trait missingness $<0.02$, 
PLINK-1.9 \cite{plink, bt_10} was applied to the base dataset
to to identify genome-wide assocations, after which clumping was performed with 
standard parameters ($R^2=0,1$, window$=250kb$) \cite{Choi2020} across the combined 
set of variants. Under the same strict threshold for genome wide significance introduced
...
were included as a covariates, but it should be noted that they explained very
...
\subsubsection*{Impact on POPout and Prediction}
...
combined rare+common PRS result.  Then for each result we calculated the $POPout$ 
in each tail, the overall variance explained ($R^2$), and the odds ratio for predicting "disease" (defined 
as the top and bottom 1\% of the trait distribution, given a PRS score in the top or bottom 1\% of the 
distribtion.  Repeating this for rare+common PRS result whenever significant $POPout$ was present 
in one or both of the tails of the common-variant PRS and one or more significant rare variants 
were present allowed us to measure POPout recovery ($\frac{{POPout}_{common}-POPout_{combined}}{POPout_{common}}$), to 
describe the improvement in the tail produced by the inclusion of rare variants (\stref{sup_rareTable}). 
ADD THE DETAILED SUPPLEMENT STUFF 


\subsubsection*{Rare Variant Annotation}

Out of a total of 610 rare variants identified above, 143 were shared across multiple traits and 67 were previously 
idenfied and had an annotation in the CLINVAR database.  These SNPs and the phenotypes they have been previously 
associated with are listed in (\stref{sup_clinVar1}). 
...
another model but is chosen because of a negligible difference in BIC.  Thus, for each pair of best 
and next best models we performed the following comparison.  When the next best model was a nested 
subset of the best model (e.g. missing a parameter) we calculated the likelihood ratio 
for the top two models and compared it the critical values from a $\chi^2$ chi-squared distribution 
whose degrees of freedom are equal to the difference in parameters for the two models to determine 
if it was significantly better ($alpha=0.05$).  When the next best 
model had more parameters we instead relied on the work of Kass and Rafferty \cite{bayesguys}, who 
determiend that a difference in BIC of less than 2 was weak evidence of model superiority. Using these 
two criteria we found 10 and 7 traits, respectively, for which there was justification to use the 
alternate model.  This traits are shown in \stref{sup_bic} where we additionally (\stref{sup_bic}{c} 
replace the top model with the alternate model for these 17 traits and reestimate the relationship 
between LRS and POPout effect observed in \fref{fig5}{d} - we observe that the expected relatipnship persists.   
