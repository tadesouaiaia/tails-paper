...
(difference between BIC values) for the top two models and compared it the critical values 
from a $\chi^2$ chi-squared distribution whose degrees of freedom are equal to the difference 
in parameters for the two models to determine if it was significantly better ($alpha=0.05$).  When the next best 
...
Analysis of our four index traits shows that polygenic prediction where assuming 
"disease state" to be the top and bottom 1\% suffers when POPout is present. 
