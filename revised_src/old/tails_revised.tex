...
We formally tested for deviations from the NULL LINEAR expectations 
by performing $t$-tests on the residuals of a regression of 
PRS on trait. Each of the middle 80 trait centiles were tested 
as a QC step, while the analysis test - which we refer to 
...
of traits that are otherwise governed by COMMON polygenic burden. 
...
IT IS CHALLENGING TO INFER SELECTION ACTING ON TRAITS FROM 
CONTEMPORARY DATA ONLY, BUT A METHOD DEVELOPED BY SANJAK ET AL WAS USED  
TO INFER ONGOING SELECTION IN THE UK BIOBANK BY MODELLING THE RELATIONSHIP 
BETWEEN TRAIT VALUES AND "LIFETIME REPRODUCTIVE SUCCESS" IN A REGRESSION FRAMEWORK. HERE 
WE EXTEND THIS APPROACH BY USING THE Bayesian Information Criteria (BIC) 
to INFER "no selection", "positive", "negative", and 
"stabilising" selection FROM the fitted models (\dref{fig4}{f}) APPLIED TO OUR 74 TRAITS (SEE METHODS). 
...
\subsection*{Rare Exome Hits and Selection Analyses} \label{sec:methods:rvs} 
...
{\color{red} 
\subsection*{Incorporating Genome Wide Rare Variants into PRS and POPout analyses} \label{sec:methods:rareVariants}

\subsubsection*{GWAS and PRS}
