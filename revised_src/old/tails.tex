...
have tails enriched for rare, large-effect alleles. 
Overall, our findings suggest that while common variants of small effect likely account 
for most of the heritability in complex traits \cite{Weiner2023}, rare variants of large 
effect are often more important in the trait tails, particularly
among individuals at highest risk of disease.
Our study has implications for rare variant discovery, the utility of polygenic scores, the study of selection in humans, 
...
We hypothesise that the strength and type of selection acting on a complex trait influences 
...
proposed by R.A. Fisher in 1918 \cite{Fisher1919} , which posits that quantitative traits result 
...
Indeed, many widely used statistical genetics methods rely on its 
...
in trait tails subject to negative selection, where genetic architecture is more heterogeneous. 
...
demonstrated that rare polygenic risk scores, calculated based on the PrimateAI-3D algorithm, 
...
its corresponding trait (\dref{fig1}{c}). However, if rare alleles are concentrated in the trait tails, then the 
PRS is expected to regress towards the mean in these extremes (\dref{fig1}{g}). We first 
...
genetic architecture of trait tails (\dref{fig1}{d}, \dref{fig1}{h}) in over 17k sibling pairs from the UK Biobank. 
...
we observe widespread and substantial departures from polygenic expectations in trait tails (\fref{fig2}, \fref{fig3}). To gain insight into the causes of these observations, we investigate whether natural 
...
we also removed individuals with cancer diagnoses or who 
...
there were 369,132 individuals of European (EUR)
ancestry available for the primary analysis.

We have developed a method that leverages polygenic risk scores (PRS), 
...
Under a null hypothesis corresponding to a polygenic model \cite{Barton2017},  
with environmental factors adjusted or jointly 
normally distributed, we expect a linear relationship between 
the trait and PRS (\dref{fig1}{c}). In contrast, if the tails are enriched for rare alleles of large 
effect, not included in the PRS derived from GWAS on 
common variants, then the average PRS in the lower and upper 
quantiles will regress to the population mean PRS (\dref{fig1}{g}). 
We formally tested for deviations from polygenic expectations 
by performing $t$-tests on the residuals of a regression of 
PRS on trait. Each of the middle 80 trait centiles were tested 
...
The 10 significant negative POPout effects could be a result 
of epistatic effects or large, non-Gaussian, 
unadjusted environmental risk factors (see Discussion). 
...
across the 28 (of 33 overlapping) traits that passed QC, also showed strong replication (\dref{fig1}{d}) of the primary 
...
departures from polygenic expectations in the tails of quantitative 
traits. Here, we investigate evidence for the role of rare variants 
and selection in causing these patterns 
...
stabilising selection about 28\% of individuals in the tails 
...
100 simulation replicates (\dref{fig4}{e}), mimicking the POPout effects 
of the real data analyses. These simulations suggest that selection can 
have a critical impact on how genetic architecture varies across the trait 
continuum and can generate substantial rare-variant architecture in the tails 
of traits that are otherwise governed by polygenic burden. 

\FloatBarrier
To test this more explicitly in our real data, we applied a 
...
Next, we stratified the POPout results of the primary analysis into 
groups of traits according to the categories of inferred selection and tested if 
...
tail for traits with inferred negative selection (lower mean = 0.08, upper mean = 0.17, $P=0.02$), 
Finally, we tested for associations between POPout effects and outcomes that reflect proxy 
measures of fitness (\dref{fig4}{h}), finding that POPout effects are significantly negatively correlated with number of 
...
or number of miscarriages and still births. 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%   END RESULTS %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%   
% 
...
performance of common PRS based for the relevant traits. 
...
UK Biobank genotype and phenotype data were obtained from the UK Biobank Resource under application 18177. \\
{(\tt https://www.ukbiobank.ac.uk/enable-your-research/approved-research/multi-trait-gwas-analyses-in-the-uk-biobank).}\\  

UK Biobank Quality Control information (missingness, allele frequency, Hardy Weinberg Equilibrium) was obtained from UK Biobank
resource 531: {\tt https://biobank.ctsu.ox.ac.uk/crystal/refer.cgi?id=531.}  
All of Us was obtained from the website. 




\section*{Software availability}
{\small Our nextflow pipeline to run our analyses as well as R code specifically to run the POPout test (on phenotype and PRS data) 
and python code to run trait QC can found here: {\tt https://gitlab.com/heiman/extreme-trait-prs} \\ 
All other code used in this study can be found on the following websites:}
{\small
\vspace*{-2mm}
\begin{enumerate}\centering
\item[PRSice-2:]{\tt https://www.prsice.info}
\item[PLINK 1.9:]{\tt https://www.cog-genomics.org/plink}
\item[PLINK 2.0:]{\tt https://www.cog-genomics.org/plink/2.0/}
\item[sibArc:]{\tt https://www.sibarc.net}
\item[LD score reg:]{\tt https://github.com/bulik/ldsc}
\end{enumerate}}
