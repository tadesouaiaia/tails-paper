...
class FigLib: 
        ...
        for i,(a,b) in enumerate([[y1,y2],[y3,y4]]): 
            while a <= b: 
                ...
                if yp*yp > MS: continue
                    ...
                    if xp < -1.5: ax.scatter(xp,yp, s=sz,alpha=0.75,color='red',lw=0.001,ec='red',clip_on=False) 
                    else:         ax.scatter(xp,yp, s=sz,alpha=0.75,color='grey',lw=0.001,ec='grey',clip_on=False) 
    ...
    def make_arrow(self, ax, a = (0,8), b = (-8,3), FLIP=False, txt='NA', STYLE='NA'):  
        ...
        ax.add_patch(arrow)
        if txt != 'NA': ax.text(a[0],a[1]+1,txt,ha='center',va='bottom',fontsize=self.fs4) 
...
class MyFigure:
    ...
    def finish(self):
        plt.subplots_adjust(left=0.04, bottom=0.005, right=0.999, top=0.88,wspace=0.5, hspace=0.02) 
    ...
    def create(self): 
        ...
        for i,ax in enumerate(self.axes):
            ...
            ax.text(x1-xs*1.5,y2,letters[i],fontsize=20,clip_on=False) 
            if i == 1: 
    ...
    def make_fake_trumpet(self, ax,NOGO=False, PLOTZONES=False, NULL=False): 
        ...
        for i,x in enumerate(X): 
            ...
            if NULL:     
                ...
                if i < 15:   self.lib.make_dots(ax, x, pts, c = clr, y_var = SD,stepsize=ss,MAX=3.5) 
                else:        self.lib.make_dots(ax, x, pts, c = clr, y_var = SD,stepsize=ss,MAX=3.5) 
        if NULL: 
            self.lib.make_dots(ax,-3.5, [-4,0,0,4], y_var = 5, c = 'red', stepsize = 0.8, MAX = 4.5) 
            self.lib.make_dots(ax,1.15, [-2.6,-2,4.1,4.5], y_var = 2, c = 'green', stepsize = 0.9, MAX = 4.6) 
            self.lib.make_dots(ax,0.8, [-2.6,-2,4.1,4.5], y_var = 2, c = 'green', stepsize = 0.8, MAX = 4.7) 
            self.lib.make_dots(ax,1.4, [-2.4,-2.3,3.9,4.5], y_var = 2, c = 'green', stepsize = 0.8, MAX = 4.7) 
        else:    
            self.lib.make_dots(ax,-3.6, [-5,0,0,5], y_var = 5, c = 'red', stepsize = 0.5, MAX = 4.75) 
    ...
    def make_fake_sibs(self, ax, fs1=29, fs2=25, rc = 'red', NULL=False): 
        ...
        if NULL: 
            txt1 = 'Common-Variant\nTail Architecture\nCauses\nRegression\nto Mean' 
            self.lib.make_arrow(ax, a = (9,10), b = (13,6), txt = txt1,FLIP=True) 
        else: 
            txt1 = '$De\,\,Novo$\nTail Architecture\nCauses Sibling\nDiscordance'
            self.lib.make_arrow(ax, a = (19,11), b = (23,6.5), txt = txt1,FLIP=True) 
        return
