...
biomedical research and precision medicine \citep{Timpson2018}. Despite much progress, 
little is known about how genetic architecture varies across the full trait spectrum and, 
in particular, if it differs in the tails of complex traits, where disease often occurs. 
Here, applying a novel approach based polygenic scores, we reveal striking departures 
from polygenic architecture across 148 quantitative trait tails. This pattern is consistently 
observed across diverse ancestries, independent cohorts, repeat measurements, and is further 
validated using an orthogonal family-based approach \cite{Souaiaia2024}. Our findings 
suggest that there are distinct concentrations of high-impact rare alleles in one or 
both tails of most complex traits. Furthermore, trait tails with inferred enrichment 
of rare alleles are associated with more exome study hits, reduced fecundity, advanced 
paternal age, and diminished predictive accuracy of polygenic scores. Finally, we find 
...
have tails enriched for rare, large-effect alleles. Overall, our results suggest that 
while polygenic burden from numerous small-effect variants  \cite{Barton2017} predominates 
overall trait variance, a substantial fraction of individuals in the trait tails may have a far simpler, 
monogenic or oligogenic architecture. Our study has significant implications for rare variant 
discovery, the utility of polygenic scores, the study of selection in humans, and for the 
relative importance of common and rare variants to complex traits and diseases.
...
effective in describing the genetic architecture of complex traits \cite{Barton2017}. 
...
make a modest contribution to overall trait variance \cite{Pathan2024}, but a substantial 
...
%\vspace*{-1mm} 
\caption{{\textbf{Two polygenic models of genetic architecture} 
...
In the “polygenic model” with strong selection: 
...
general (\dref{fig2}{b}, Supplemental Figures). Of the 107 tails with significant 
...
%and how weak and strong selection are simulated see the \myemph{Methods: Simulation}. 
...
data and a polygenic quantitive ‘trait’ calculated as the sum of genome-wide alleles, 
...
coefficient > 0.044) \cite{erase}.  After these QC steps, 411,949 unrelated    % THESE NUMBERS DON'T ADD UP 
...
insufficient statistical power of the non-European ancestry samples \cite{Souaiaia2024}. 
...
with heritability $h^2<5\%$ \cite{Choi2020}, while to ensure that the traits were 
...
composition related) with pairwise $r_g<95\%$.  In addition to 
...
mean. Following Sanjak et al \cite{Sanjack2018}, the presence and
...
population of N=10,000 diploid individuals of 100kb was simulated to
...
the sum of all genetic effects, which full heritability ($h^2 =1$).
...
\end{equation}
where $k$ is a multiplier for strength of selection (eg. weak selection ($k=1$) or 
strong selection ($k=100$)),
...
We ran this simulation for 100 independent runs and colleted data in 
each simulation by logging, at each generation, the phenotypic mean and standard 
deviation of the population, statistics regarding the mutations present in the 
populations, including allele frequencies and effect sizes, and the mutational profile of each individual. 
To analse the enrichment of rare variant architecture at different quantiles, the population 
was split across 100 centiles (each including 100 individuals) and the number of rare and common 
mutations carried by individuals in each centile was calculated under neutrality vs under selection.  
The mean of the log-transformed odds ratio across all 100 independent simulations was used as a measure 
of enrichment and to test for significant differences from zero (using a t-test, see \dref{fig4}{d}).  
To analyse how the impact that low-frequency variants have on prediction and mimic common 
variant PRS, a second genetic value was calculated using only variants with \emph{MAF} $>1\%$ 
and compared to the true value across the individuals in each centile.  The mean difference (POPout) 
and 95\% confidence intervals in each centile were calculated across the 100 independent simulations (\dref{fig4}{e}) 
...
-terminology on page 8: negative versus positive selection reads confusing here because it is usually used in the 
context of selection on genetic variants. You could say “negative directional selection” and 
“positive directional selection” to emphasize that we are talking about selection on the phenotype and not on the variant.
