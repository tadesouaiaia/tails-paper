...
Understanding the genetic architecture of human traits is critical for advancing 
biomedical research and precision medicine \cite{Timpson2018}. Despite much progress, little is known about how genetic architecture varies across the full trait spectrum and, in particular, if it differs in the tails of complex traits, where disease often occurs. Here, applying a novel approach based polygenic scores, we reveal striking departures from polygenic architecture across 148 quantitative trait tails. This pattern is consistently observed across diverse ancestries, independent cohorts, repeat measurements, and is further validated using an orthogonal family-based approach \cite{Souaiaia2024}. Our findings suggest that there are distinct concentrations of high-impact rare alleles in one or both tails of most complex traits. Furthermore, trait tails with inferred enrichment of rare alleles are associated with more exome study hits, reduced fecundity, advanced paternal age, and diminished predictive accuracy of polygenic scores. Finally, we find evidence of ongoing selection consistent with the observed departures in polygenicity and demonstrate, via simulation, that traits under stabilising selection are expected to have tails enriched for rare, large-effect alleles. Overall, our results suggest that while polygenic burden from numerous small-effect variants  \cite{Barton2017} predominates overall trait variance, a substantial fraction of individuals in the trait tails may have a far simpler, monogenic or oligogenic architecture. Our study has significant implications for rare variant discovery, the utility of polygenic scores, the study of selection in humans, and for the relative importance of common and rare variants to complex traits and diseases.
...
wide range of complex traits \cite{Flannick2019, Singh2022}. The number of variants 
...
Stabilising selection, a key driver of genetic variation \cite{Oconnor2019, Ori2024}, 
reduces fitness in individuals whose trait values deviate from an optimal mean, which consequently limits the frequency of large-effect alleles. Despite this, most statistical genetics methods 
...
We hypothesise that the strength and type of selection acting on complex trait influences the distribution of common and rare alleles along the trait continuum. Specifically, we propose that many complex traits have experienced sufficiently strong selection to generate marked enrichments of large-effect, rare alleles in the trait tails (Fig.1). This concept contrasts with the classical infinitesimal model, initially proposed \cite{Fisher1919} by R.A. Fisher in 1918, which posits that quantitative traits result from the cumulative effects of numerous alleles, each one of tiny effect. This “polygenic” model has since been extended to explain genetic liability of complex diseases and has proven incredibly effective in describing the genetic architecture of complex traits \cite{Barton2017}. Indeed, many widely used statistical genetic methods rely on its assumptions \cite{Yang2011, BulikSullivan2015, Choi2020}. Our hypothesis diverges from the infinitesimal model to the extent that selection constrains large-effect alleles to remain at low frequencies, effectively concentrating rare alleles in the tails of complex traits (Fig.1). Thus, while the infinitesimal model accurately reflects the genetic architecture for the majority of the population, where polygenic burden predominates, it may be a poor approximation in trait tails under negative selection, where genetic architecture is more heterogeneous. This would be a key caveat to the infinitesimal model because in this case rare variants may make a modest contribution to overall trait variance \cite{Pathan2024}, but a substantial contribution in trait tails, which are of critical evolutionary and clinical significance  \cite{Kingsolver2007, Ference2018, Kyrou2000}. 
...
were more predictive for individuals with extreme, rather than average, trait values. However, so far there has been no systematic investigation into how genetic architecture varies across complex trait distributions and whether, in particular, the tails of quantitative traits have distinct genetic architecture. 
...
%\vspace*{-1mm} 
\caption{{\textbf{Schematic of trait characteristics
...
In this study, we apply multiple approaches to interrogate how genetic architecture varies across the trait continuum. First, we introduce the PRS-On-Phenotype outlier (“POPout”) test, a novel method that uses polygenic risk scores \cite{Choi2020} (PRS) to detect deviations in polygenic expectations in the tails of trait distributions. The premise of the POPout test is that a PRS derived from common variants should exhibit a positive, linear correlation with its corresponding trait. However, if rare alleles are concentrated in the trait tails, then the PRS is expected to regress towards the mean in these extremes (Fig1c). We first apply POPout to 74 quality-controlled quantitative traits in individuals of European ancestry from the UK Biobank \cite{Bycroft2018}, followed by replication in individuals of multiple ancestries from the UK Biobank and in the US-based All of Us cohort \cite{Bick2024}. Next, we employ an independent method \cite{Souaiaia2024} 
that uses sibling trait variability to infer the genetic architecture of trait tails (Fig.1d) in over 17k sibling pairs from the UK Biobank. This sibling-based approach provides complementary evidence to POPout, since it leverages non-genetic family data to infer tail architecture. Across these approaches and cohorts, we observe widespread and substantial departures from polygenic expectations in trait tails (Fig.2, Fig.3). To gain insight into the causes of these observations, we investigate whether natural selection may drive these findings  (Fig.4). Extending a method \cite{Sanjak2018} that uses lifetime reproductive success to infer ongoing positive, negative, and stabilising selection, we find consistency between the type of selection inferred and the tail(s) of a trait that appear to be selected against according to our POPout results. Supporting this, simulations using the forward-in-time simulator SLiM \cite{BC_2019}, show that stabilising selection results in POPout effects in both tails, whereas traits simulated under neutrality display no such effects. Finally, we observe that trait tails with POPout effects are associated with a higher frequency of significant exome-based rare variants, reduced fecundity in both males and females, and advanced paternal age. Our findings reveal the distinct nature of genetic architecture in the tails of complex traits, highlighting the importance of distinguishing between the diverse genetic profiles that can lead to extreme trait values in different individuals and in different traits.
...
After quality control (QC) steps (see Methods), to ensure, for example, that the traits selected for analysis are heritable, approximately normally distributed, are heritable and sufficiently independent, 74 traits remain for analysis. To limit the contribution of environmental factors, we 
...
we expect a linear relationship between the trait and PRS (Fig.1c). 
...
\end{maybe} 
\caption{{{\bf Population based POPout results.}
...
general (Fig.2b, Supplemental Figures). Of the 107 tails with significant 
...
Here we perform three different forms of replication to test the generalizability of our primary findings that used UKB European ancestry data. In each replication dataset, we only include traits that pass our POPout QC (see Methods) and have sample size over 5k individuals based on inferred power (see Supplemental Fig.1). First, to investigate if POPout effects of the primary analysis are due to residual non-genetic factors not captured by our QC or covariate adjustments, we repeated the analysis in a held-out sample of individuals that had traits measured at baseline and at approximately 5-year follow-up. By taking mean values of the two repeat measures, the impact of transient, environmentally influenced factors, such as infections, short-term medication use, or diet, should be minimized. This ‘UKB Repeated’ analysis, performed on up to $\sim$15k individuals across the 50 overlapping traits that passed QC, showed a strong and significant ($R=0.71$, $P=2.3 \times 10^{-16}$) correlation with the results of the primary analysis (Fig.2d), suggesting genetic contribution to the observed POPout effects of the primary analysis. Next, we investigated the generalizability of our findings to a held-out sample of UKB multi-ancestry ancestry individuals (see Methods). This ‘UKB Multi-Ancestry’ analysis, performed on up to $\sim$22k individuals across the overlapping 54 traits that passed QC, produced highly correlated and significant replication ($R=0.63$, $P=3.7 \times 10^{-13}$) of the primary results (Fig.1d). Finally, we apply the same approach to European ancestry individuals from the US-based All of Us data \cite{Bick2024} to evaluate the generalizability to matched-ancestry sample but from a clinical-based cohort from a different continent, with different collection and assaying procedures, and a different healthcare system and set of environment factors. This ‘All of Us’ analysis, performed on up to $\sim$127k individuals across 28 traits, also showed strong replication (Fig.1d) of the primary analysis results ($R=0.59$, $P=1.6 \times 10^{-6}$). See Supplementary Material (Supplemental Fig.1) for a full breakdown of all replication analyses. 

%
%
\subsection*{Tail architecture inferred from siblings} 
Next, we seek to validate our findings further using family data. We have developed a method \cite{Souaiaia2024} that evaluates trait data in large samples of siblings to infer genetic architecture in the tails of quantitative traits. This provides an alternative, complementary approach to infer tail architecture from independent, family-based, non-genetic data. The intuition of the approach (Fig.3a) is that individuals with extreme trait values have siblings whose trait values reflect the architecture in the tails, such that under: (1) polygenic architecture: siblings have high but less extreme trait values, (2) de novo architecture: siblings have trait values reflective of the general population, (3) Mendelian architecture (i.e. due to rare, high-impact alleles that segregate in families): siblings either have similarly extreme trait values or have trait values that reflect the general population.
...
The UK Biobank (UKB) includes over 17k sibling pairs of European ancestry available for our STANDout analyses, none of which were included in our population-based analyses. There were too few sibling pairs from the multi-ancestry sample for a sufficiently powered multi-ancestry sibling analysis. Data across all 74 traits were available for analysis. Replicating our POPout analysis, our STANDout analysis showed no departure from polygenic expectations for phosphate, showed a significant lower tail STANDout effect for haemoglobin concentration ($P_{lower}=2 \times 10^{-5}$), a significant upper tail STANDout effect for red blood cell distribution width ($P_{upper}=4 \times 10^{-4}$), and significant effects in both tails for sitting height ($P_{lower}=7 \times 10^{-4}$, $P_{upper}=3 \times 10^{-8}$; see Fig.3b and Fig.2a for comparison). More broadly, our sibling-based STANDout analyses replicated our population-based POPout analyses across the 74 traits extremely well (Fig.3c; $R = 0.66$, $P = 3 \times 10^{-20}$). This is reassuring given that the two methods use such orthogonal approaches and types of data. Further reassurance of the validity of our sibling-based method is provided by the strong correlation ($R = 0.81$, $P = 2 \times 10^{-9}$) between SNP-based heritability ($h^2$), estimated by the popular LD score regression approach~\cite{BulikSullivan2015}, and the sibling-based $h^2$ estimated as a key parameter in the STANDout approach (Fig.3d).
...
(Fig.1). Backman et al. \cite{Backman2021} conducted 
...
positive and twenty with negative effects (Fig.4a). 
...
effect (Fig.4b). This provides direct 
...
\end{maybe} 
\caption{{\bf Concordance between POPout effect size and
rare variants of large effect and selective pressure.}
\textbf{(a)} Trumpet plots (minor allele frequency v effect size) of
genome-wide significant variants for the four traits selected for their 
different POPout characteristics (see Fig 2a).  
\textbf{(b)} POPout effect size plotted against the number of 
genome-wide significant variants with MAF<1\% for 58 traits with overlapping exome sequence
...
\textbf{(c)} Age of menopause: PRS quantile vs. trait mean and trumpet plot.
\textbf{(d)} Simulation: Enrichment of variant-type by trait quantile under different environments. 
Rare and Ultra Rare correspond to MAF<1\% and MAF<0.05\%, respectively.  Large denotes variants in the top 
10\% of absolute effect. For more information on the variant distribution 
and the how weak and strong selection is simulation see the \myemph{Methods: Simulation}. 
\textbf{(e)} Simulation: $\Delta$ PRS refers to gain between traditional common variant PRS (MAF>1\%) and 
a theoretical PRS where all variants are known and used. 
\textbf{(f)} Residualised trait value plotted against relative
...
traits. 
\textbf{(g)} POPout effect size grouped by model selection
category for 10 traits under no selection, 8 traits under non-direction 
stabilising selection, 29 traits under positive selection, and 27 traits under negative selection. 
Significant difference between upper and lower tail mean effect sizes calculated using a two-sample t-test. 
\textbf{(h)} Spearman rank correlation between four proxy measures of fitness and
...
genetic parameter settings (Methods), we simulated genetic variation data and a ‘trait’ calculated as the sum of genome-wide alleles, with effect sizes drawn from a Gamma distribution. After an initial burn-in period under neutrality, 
...
after strong stabilizing selection (Fig.4d). Under 
...
only across replicated (Fig.4e). These simulations 
...
"stabilising" selection in the fitted models (Fig.4f). 
...
the POPout effects reflect the inferred selection (Fig.4g). 
...
Finally, we tested for associations between POPout effects and proxy measures of fitness (Fig.4h), finding that POPout effects are significantly negatively correlated with number of offspring (paternal: $P=0.0001$; maternal: $P=0.0233$), 
significantly positively correlated with paternal age ($P=0.0096$) and not significantly associated with self-identified illnesses or number of miscarriages and still births. Notably, the well-established increase in \emph{de novo} mutation rate with advanced paternal age\cite{Kong2012} is consistent with the corresponding 
...
effects (Fig.2a). Likewise, a priori evidence of enrichment of de 
...
Our study had a number of limitations and areas for future follow-up. Firstly, and most important, we cannot rule out the contribution of 
...
we believe that enrichment of high-impact rare alleles is a major cause of our findings. Future work should interrogate potential genetic and environmental causes of extreme trait values at the individual level, with greater trait-specific attention than possible from our trait-wide analyses. Secondly, in our multi-ancestry replication analysis we combined individuals of many diverse ancestries into a single group, which poorly reflects global diversity and prevents evaluation of potential heterogeneity across populations. However, relatively large sample sizes are required to study tails of trait distributions in this 
...
global samples. Thirdly, we did not explicitly integrate rare PRS to recover observed reductions in absolute PRS in the tails. This was due to the recent publication of a highly comprehensive study that performs a similar analysis and provides direct support for our findings \cite{Fiziev2023}, 
...
African, 2,585 West Asian, 2,550 East Asian, and 1,619 multi-ancestry
samples.
...
insufficient statistical power of the non-European ancestry samples \citep{Souaiaia2024}. 
...
residualised within each ancestry group (European and Non-European Multi-Ancestry) by applying a linear regression 
...
%
\paragraph*{Non-European Multi-Ancestry Individuals}
...
The All of Us Research Program (All of Us) is a multi-ancestry biobank from the United States, utilising 
...
\subsubsection*{POPout regression QC}
...
\subsubsection*{Evaluating Impact of POPout Effects on PRS Prediction} 
...
\clearpage \newpage
\note{NEW SECTION:} 
\subsubsection*{Estimating Variance Explained by Rare Variants from POPout Effect} 
To estimate the proportion of trait variance described by unaccounted for 
rare variants we make a number of assumptions.  First we assume that rare 
variants are "monogenic like" meaning that the expected common variant 
genotype (polygenic risk) of extreme individuals with rare variants is equal 
to the population mean ($\mu_R$).  Then for every percentile in the lower and upper 5\% of 
the distribution where POPout is present we estimate the fraction of individuals 
who came from this distribution: 

\begin{align*}
    PRS_{obs} = \frac{k}{n} \mu_R    + \frac{n-k}{n} PRS_{expected} 
\end{align*}

Next, we simulate a trait whose values are given by $T=rG + E$ where G represents total genetic risk 
(standaridised to mean 0 and variance 1) and the environmental effect E is drawn from a 
normal distribution with mean 0 and variance (1-$r^2$), ensuring a standard normal trait distribution. 
For any trait, we can assign an initial $r^2$ value that representing variance explained by total 
genetic risk and simulate the corresponding genotype and phenotype for 500,000 individuals. Then in each 
tail percentile, we can randomly select $\frac{k}{n}$ individuals, assign them a gentic contribution drawn 
from a standard normal, and recalculate the association between the remaining common genetic risks and the 
phenotypes ($r^2_{common}$) and the fraction explained by rare variants alone ($r^2 - r^2_{common}$).  
 
Iteratively increasing $r^2$ until $r^2_{common}$ is equal to the value for $r^2$ that we calculated 
using LD score regression \cite{ldsc} allows us to estimate what the total trait variance explained would 
have been had POPout not been present and estimate the fraction of variance explained by known (common) and 
rare variants. 
...
We have already developed sibling-based tests of departures from polygenic expectations in trait tails caused by \emph{de novo} high-impact alleles or caused by rare alleles of large effect segregating in families ("Mendelian" effects) \cite{Souaiaia2024}. Here we summarise the theoretical framework underlying those tests, and then describe the novel test used here that combines the two tests into a single omnibus test to identify general departures from population expectations due to either cause. \\
The conditional distribution of an individual's trait value given their sibling's trait value, assuming the infinitesimal model for genetic effects ("polygenicity") and normally distributed environmental factors, can be derived analytically \cite{Souaiaia2024} as:
%
...
hypotheses of $H_1: z>0$ and $H_1: z<0$ are tested as one-tailed t-tests, respectively. For further details, see Souaiaia et al \citep{Souaiaia2024}.
...
\note{NEW SECTION:} 
\subsubsection*{Inferring Detailed Tail Architecture from Sibling Data} 

The \textit{SibArc} software package \cite{Souaiaia2024} includes algorithmic routine to 
estimate the fraction of tail architecture attributed to polygenic, \textit{De Novo}, 
or Mendelian Architecture.  This routine assumes that for standardised complex trait, the 
under polygenic and \textit{De Novo} architecture that the conditional sibling distribution 
given one sibling ($2_1$) in the tail of the distribution is $\N(0,1)$ and 
$\N(\frac{1}{2}s_1 h^2,1 - \frac{h^4}{4})$, respectively.  Under pure Mendelian architecture,  
the conditional distribution giving a sibling in the top 1\% of the distribution depends on 
whether the siblings share a familial variant and is either standard normal or truncated normal: %we assume happens with equal probability: 

\begin{align*}
P(s_2 | s_1, \text{Mendelian Arch}) = 
\begin{cases}
    Z & \text{if unshared} \\
    \frac{\varphi(2.326)}{Z} & \text{if shared}
\end{cases} 
\end{align*}

Assuming that both cases occur with equal probability the conditional sibling 
probability distribution function given tail Mendelian and $De Novo$ fractions of $a$ and $b$, is: 

\begin{align*}
    a\frac{1}{2}\left(p(s_2 | S_2 = Z)  + p(s_2 |  S_2 \frac{\varphi(2.326)}{Z} \right) + b(s_2| S_2 = Z) + (1-(a+b))(s_2 | S_2 \sim \N(\frac{1}{2}s_1 h^2,1 - \frac{h^4}{4}))
\end{align*}

This routine iteratively identifies the $a$ and $b$ that maximize this likelihood across all siblings, subject to $a+b<=1$.  
Then, $a,b,c$ represent the most likely fraction of Mendelian, $De Novo$, and polygenic architecture given a set of conditional siblings. 
...
\FloatBarrier \begin{figure}[!h] \begin{maybe}{showFig} 
\includegraphics[trim=0cm 0cm 0cm 0cm, clip, width=\linewidth]{Figures/Sup2.pdf} \end{maybe} 
\caption{{{\bf Variance Explained by Rare Variants}
Rare Variant $R^2$ Plotted Alongside Variance Explained by Common Variants 
}} \label{sup2} \end{figure}
\FloatBarrier

\FloatBarrier \begin{figure}[!h] \begin{maybe}{showFig} 
\includegraphics[trim=0cm 0cm 0cm 0cm, clip, width=\linewidth]{Figures/Sup2b.pdf} \end{maybe} 
\caption{{{\bf Rare Variant Heritability}
Rare Variant Heritability Plotted Alongside Common Variant Heritability 
}} \label{sup2b} \end{figure}
\FloatBarrier

\FloatBarrier \begin{figure}[!h] \begin{maybe}{showFig} 
\includegraphics[trim=0cm 0cm 0cm 0cm, clip, width=\linewidth]{Figures/Sup3.pdf} \end{maybe} 
\caption{{{\bf Detail Tail Architecture Inferred from Sibling Data}
For all traits with at least one nominally significant \textit{STANDout} tail test, the estimated 
fraction of polygenic, $De Novo$, and Mendelian Architecture is shown in gray, red, and purple.  
}} \label{sup3} \end{figure}
\FloatBarrier
