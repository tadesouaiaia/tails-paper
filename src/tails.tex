...
\fref{fig:intro}).  Next we consider whether evidence of negative selection
...
the mean risk score than expected.  To quantify this phenomenon we
develope a quantitive trait statistical test that estimates effect
size, direction, and signficance of movement away from the expected
polygenic load for individuals in the lower and upper 1\% of the trait
distribution. Specifically, a regression is fit... The
regression effect size is defined as
\[
  \beta = \mbox{abs(expected mean trait value in tail) -
    abs(observed mean trait value in tail)}
\]
where the the expected mean trait value in tail is calculated from the
regression fit. Using this definition, positive
values in both the upper and lower tails correspond to regression to
the mean, whereas negative values correspond to regression away from
the mean.
...
poor polygenic prediction ($R^2$), we identify a set of 75
high-powered ($h^2>0.05$,$R^2>0.01$) non-redundant, quality-controlled
traits for further analysis (see Methods - QC).  Using these 75 traits
we perform PRM-Testing, rare-variant analysis, analysis of selection
to help elucidate the mechanism that may be behind PRM.  We then
replicate these results using a sibling model and different datasets.
...
observed in each of these traits can be illustrated in
\dref{fig:prs}{a}.
...
\caption{\footnotesize{{\bf Population-level data analyses for rare
    ...
    Regression, Upper Tail, Lower Tail, Dual Tail). \textbf{(b)} PRM
    effect sizes in the upper and lower 1\% tails for all traits (of
    the 77 passing trait QC) which exhibit significant regression to
    the mean in one or both tails (Bonferroni p-value<0.05 $\equiv$
    P<0.05/154).  \textbf{(c)} Trumpet plots -- minor allele frequency
    plotted against effect size of genome-wide significant variants
    for traits shown in (a). \textbf{(d)} Tail effect sizes plotted
    against number of genome-wide significant variants with MAF<1\%
    for X traits with overlapping exome sequence results. For upper
    tails positive variant effects are counted and for lower tail
    negative variant effects are counted. Correlation and significance
    calculated using Spearman rank correlation. \textbf{(e)} Age of
    menopause: PRS quantile v trait mean and trumpet plot.
...
\caption{\footnotesize{{\bf Selection analyses in real and simulated
    ...
    relative lifetime reproductive success for 16 {\bf cherry picked
      traits}, quadratic best fit to the data is shown by dashed
    lines.  \textbf{(b)} Number of traits with inferred type of PRS
    tail regression and inferred type selection for 75 UKB traits
    passing QC. Tail regression is inferred if P-value for regression
    to the mean in the tail is significant... \textbf{(c)} Spearman
    rank correlation between the standardised mean proxy measures of
...
\noindent Using adjusted lifetime reproductive success
\citep{v_select, v_select2} as a proxy for selection we classify (see
Methods - LRS) each trait with respect to whether it is under
...
(55 of 75) are classified as under stabilizing selection which is
characterized by lower fecundity in both tails of the trait
distribution. Stabilizing selection may be the result of general
fitness favoring the trait mean or the result of sex specific
...
\FloatBarrier \begin{figure}[!h] 
    ...
    \caption{\footnotesize{{\bf Sibling analyses for rare variants of
        ...
        P-values plotted against sibling meta test P-values on $-\log_{10}$
        scale for upper and lower tails of X traits.}}
...
\FloatBarrier \begin{figure}[!h] \begin{maybe}{showFig} \includegraphics[trim=0cm 0cm 0cm 0cm, clip, width=\linewidth]{Figures/Fig5_eur_qc_0_version1_dedupe.pdf} \end{maybe} 
  \caption{\footnotesize{{\bf Replication results in UKB non-European
      ...
      ancestry samples for all 25 traits overlapping with All of US
      and pass PRS-test quality control in the three cohorts.
      \textbf{(c)} Simulated Signal Replication: Increasing larger
      subsets were sampled and the replicability relative to our UKB
      result was calculated \textbf{(d)} Same thing but
      ...
      size for the trait. Pearson correlation shown\textbf{(g)}. \textbf{(h)} PRS-test effect
      sizes estimated in: UKB European ancestry samples v All Of Us
      European ancestry samples for 24 overlapping traits passing
      PRS-test QC in both cohorts. Triangles shaded relative to the
      All of Us sample size for the trait.}
...
samples and replication analyses was performed on the remaining 22k
samples classified as non-European ancestry.
...
This futher thinned our traits to 123. To ensure a non-redundant
survey of traits, pairwise genetic correlation were calculated using
LDscore\cite{ldsc} and 61 traits did not have a pair with
$r_g > 95\%$, average top match = $0.60$,standard-deviation = $0.2$)
and kept in the analysis.  Additionally, 10 traits has a single almost
identical match (eg: "Standing Height" (ID:50) and "Height Measured
Prior to Imaging" (ID:12144)), in these cases the trait with greater
sample size was priotized.  The other 52 traits were part of larger
clusters of genetically similar traits, the largest of which related
to measurements related to of body-mass (eg: body fat percentage, leg
mass, arm mass, etc) and heel bone mineral density.  Using the same
data maximization algorithm applied to extra unrelated individuals
\citep{erase}, we were able to select 11 of these traits which
resulted in a set of 77 genetically non-redudant traits.

\subsubsection*{Non-European Ancestry Individuals in the UKB} ~\\
\noindent Non-European Ancestry samples in the UKB have between
3,150 and 13,037 (average 10k) total samples for our 75 QC-validated
traits.  Applying our PRM-test using the European Ancestry PRS to the
Non-European target, 71 traits passed our PRM-QC. For these 142 tails
we calculated the replication accuracy and replication association as
described above.
...
\noindent All Of Us in biobank contains XX binary and XX continuous
traits from individuals of multiple ancestries.  Trait matches were
found with an initial database search and then were inspected by eye
to determine if trait descriptions were sufficiently similar.  This
process yielded 32 traits, 25 that passed our PRM-QC. These traits have
between 8595 and 127677 observations (average 73k) in the European
ancestry samples.  For these 50 tails the replication accuracy and
replication association was calculated as described above.
...
%%%%%%%%%%%   TAIL FITNESS   %%%%%%%%%%%%
\subsubsection*{Measuring Fitness in the Tails} ~\\ 
...
has been used previously to simulate genetic effects in humans; it
reflects a prior belief in a small number of large effects and many small
effects. Trait values were determined by the sum of all genetic
effects.
...
\end{equation}
where x is the phenotype of the individual at a specific generation t,
$\mu$ and $\sigma$ are the trait meand and standard deviation at the
start of selection (i.e., $t=10N$). The fitness of each individual was
calculated and updated at the end of each generation. Stabilizing
selection was simulated until generation 11N after which the
