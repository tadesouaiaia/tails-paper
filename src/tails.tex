...
After genetic QC, we initially selected all quantitve traits from 
categories \textit{Biomarkers}, \textit{Physical Measures}, 
the \textit{Touchscreen Questionnaire} and \textit{Cognitive Tests} 
to provide a broad group of traits likely to have polygenic etiology.  
We then removed 34,698 cancer cases, 1311 individuals on insulin, and 109 pregnant 
women from all traits.  We also removed ~65k individuals on statins and other cholesterol lowering drugs from 
the \textit{Blood count} and \textit{Blood Biochemistry} subcategories and removed ~30k individuals on heart 
...
from the mean.  After these removals, we defined quantitive traits as those which contained at least ten 
distinct values and whose top two modal values comprised less than half the samples. All such traits with at least 
$100,000$ total samples of European ancestry and at least $1000$ sibling pairs of European ancestry were then 
selected and normalised as described below.  
...
model to each trait, with covariates for $age$, $age^2$, $age^3$, sex, menopause status, 
type-II diabetes status, coronary artery disease status, alcohol use (categorial and continuous), 
smoking (categorical and continuous), batch, centre, and the first 40 genetic principal components.  
At this stage, traits whose residuals had absolute skew $>2$ in the larger European ancestry group were 
removed to avoid measurement bias and remaining trait values were standardized using a rank inverse 
normal transform on residuals to provide standard normal distributions for all initial analyses. 
...
To ensure that the traits were meaningly different from each other, we considered the 
...
performed 100 trials across each of the 148 trait tails and calculated
...
We replicated $POPout$ test results were replicated in three datasets, 
\textbf{(1)} UKB European ancestry repeated measures, 
\textbf{(2)} the UKB Non-European ancestry cohort, and 
\textbf{(3)} the European ancestry samples in All Of Us.  In each dataset
matching traits with at least five thousand samples that passed the 
\textit{$POPout$ regression QC} mentioned above were included and the 
Pearson correlation across the $POPout$ tail effect sizes was calculated 
to quantify replication.  
