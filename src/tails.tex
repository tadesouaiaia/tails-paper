...
\caption{\footnotesize{{\bf Selection analyses in real and simulated
    ...
    relative lifetime reproductive success for 16 example traits, quadratic best fit to the data is shown by dashed
    lines.  \textbf{(b)} Model selection on lifetime reproductive success is performed using BIC, most traits are assigned 
    to a quadratic model with a negative gamma parameter - stabilising selection. 
    \textbf{(c)} Directionality of selection paramter is associated with tail regression in the opposite direction.  
    Traits estimated to be under negative selection have greater negative PRM (pvalue $2*10^-7$).  Traits under 
    positive selection have negative PRM (pvalue $0.02$).   
    \textbf{(d)} Spearman
    rank correlation between standardised mean proxy measures of
    fitness of individuals in the upper and lower 1\% tails and
    PRS-test effect size for the corresponding tail across 74 traits
    in UKB passing QC.  
    \textbf{(e)} \need{Coming Soon} Trumpet plots (minor allele frequency v
...
\FloatBarrier \begin{figure}[!h] 
    ...
    \caption{\footnotesize{{\bf Sibling analyses for rare variants of
        ...
        traits. Correlation calculated using pearson correlation.}}
