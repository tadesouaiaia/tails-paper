...
Primary analyses were performed on the over 410k unrelated European 
samples that comprised the largest ancestry cluster and replication analyses 
were performed on the remaining 22k samples that represented multiple 
non-European ancestry ancestries. 
...
coefficient within the range 0.177 and 0.354, this range on either 
side of the expected value of 0.25 was used to account for the 
expected variation in within-pair similarity \citep{sib1}. The proportion 
of SNPs with zero identity-by-state (IBS0) was used to distinguish
sibling pairs from parent-offspring pairs since parent–offspring pairs
have IBS0$\approx$0. As it has been shown in the UKB that there 
is good separation between pairs of first degree relatives at
IBS0=0.00112 \citep{sib1}, sibling pairs were selected as those
exceeding this threshold, this gave a total of 17,869 sibling
...
from the mean were removed (???? DOES THIS HAPPEN BEFORE OR AFTER WE REMOVE SAMPLES - SHOULD BE AFTER?). 
Traits were then standardised as follows:
...
Samples were then split randomly into 50\% base and 50\% target data
sets and GWAS and PRScice were applied and our PRM-regression test was
ran on all 1,158 continuous traits (see Supplementary Material).
Strong association between target sample size and nominal PRM
signficance ($R=0.5$, p-value$<10^{-70}$), suggests that many of our
tests were underpowered.  For this reason we focused all primary
analyses on the 136 traits with 50k target samples and PRS $R^2>1\%$.
(%new correlation = 0.2, pvalue = 0.012%)
Furthermore, our test assumes that transformation to continuous
Gaussian distribution is appropriate and that the underlying genetic
architecture is primarily polygenic, therefore we impose the following
...
1. For each trait, the residuals from the regression are then split
...
different to zero.  The tests were applied to individiuals in the top
...
involves splitting samples into four birth cohorts (1934-42, 1943-48, 1949-55,
...
($Y_{rLRS}$). Sanjak et al \citep{v_select} relied on this method to 
infer the presence and direction of selective pressure acting on the 
trait by fitting the following regression model: 
\begin{equation}
...
parameters.  Here, we are interested in whether negative selection 
enriches highly penetrant rare variants and reduces polygenicity.  
Thus, to classify whether negative selection is acting in each trait tail 
we use AIC to select the top performing sub-model of \eref{math:LRS} 
under the following framework: 


\begin{itemize}
    \item \textbf{$Y_{rLRS} = \beta_o$} (No Selection)  
        \begin{itemize} 
        \setlength{\itemindent}{1em}
        \item If the selected model does not include a linear or quadratic parameter, then the traits 
              is classified as under no selection and rare variant enrichment is not expected in either tail. 
        \end{itemize} 
    \item \textbf{$Y_{rLRS} = \beta_o + \beta{z}$} (Linear Selection) 
        \begin{itemize} 
        \setlength{\itemindent}{1em}
        \item If the selected model includes only a linear parameter, then $\beta<0$ indicates 
              negative selection and rare variant enrichment is expected in the upper tail, while $\beta>0$ indicates 
              positive selection and rare variant enrichment is expected in the lower tail. 
        \end{itemize}
    \item \textbf{$Y_{rLRS} = \beta_0 + \gamma z^2 $} (Quadratic Selection) 
        \begin{itemize}
            \setlength{\itemindent}{1em}
            \item If the selected model includes only a quadratic parameter, then $\gamma<0$ indicates 
              stabilizing selection and rare variant enrichment is expected in the both tail, while $\gamma>0$ indicates 
              divergent selection and rare variant enrichment is expected in the neither tail. 
        \end{itemize}
    \item \textbf{$Y_{rLRS} = \beta_0 + \beta{z} + \gamma z^2 $} (Full Model) 
        \begin{itemize} 
        \setlength{\itemindent}{1em}
        \item If the selected model includes both a linear and quadratic parameter, then, if $\gamma<0$ and $\beta<0$  
              selection is both stabilizing and negative and greater rare variant enrichment is expected in the upper tail, 
              if $\gamma<0$ and $\beta>0$ selection is stabilizing and positive and greater rare variant is expected in the 
              lower tail.   If $\gamma>0$ then rare variant enrichment is not expected in either tail. 
        \end{itemize}
\end{itemize} 

%To determine the direction and degree of selection 
%in each tail we fit the data to four sub-models and used selected the 
%best performing model using AIC: 


%The pressence and type of selection is inferred from the
...
%  (i.e. negative selection is not occurring in either tail).
%\end{itemize}
...
The following proxy measures of fitness were tested for association
with PRM in tails:
\begin{itemize} 
...
\item{\textbf{Paternal Age:}} Paricipant age (Field 21022) is
  ...
  at birth.
\end{itemize}
Association between these measures and PRM was calculated as
follows. For each trait, the mean value of the four proxies of
selection are calculated for individuals in the upper and lower 1\% of
the trait distribution separately. Association is then measured by
Spearman's rank correlation between the mean tail values for each
proxy and the PRM effect size across all trait tails.
