...
from population level analyses. The
...
\caption{\footnotesize{{\bf Population-level data analyses for rare
    ...
    PRS-test tail effect sizes in the upper and lower 1\% tails for
    all traits (of the 75 passing QC) which exhibit significant
    regression to the mean in one or both tails at FDR of 0.05.
    \textbf{(c)} Trumpet plots (minor allele frequency v effect size)
    of genome-wide significant variants for traits shown in
    (a). \textbf{(d)} PRS-test tail effect sizes plotted against
    number of genome-wide significant variants with MAF<1\% for X
    traits with overlapping exome sequence results. For upper tails
    positive variant effects are counted and for lower tails negative
    variant effects are counted. Correlation and significance
