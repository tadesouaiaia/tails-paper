...
\clearpage 
\newpage
After genetic QC, quantitative, unimodal, non-skewed traits were
selected for analysis as follows: at least 10 distinct values, top two
modal trait values include less than half of the samples, dip-test
(\citep{dip}) P-value for unimodality $>$ 0.05 and skew $<$2.  To
ensure our analyses were well powered, traits were further thinnned by
selecting those with heritability $h^2>1\%$ (measured by LD score
\citep{ldsc} applied to publicly available UKB GWAS summary statistics
\citep{neale}) and $>$50,000 observation in unrelated
samples. Furthermore, we selected polygenic traits by removing traits
which had a single variant $r^2$ > 0.5\%, leaving 123 traits.
...
did not have a pair with $r_g > 95\%$ and were kept for analysis.
...
In the selected 77 traits, values more than six standard deviations from
the mean were removed.  Traits values were then standardised as follows: (1)
linear regression models were applied to each trait, with covariates
for age, sex, recruitment centre, genotyping batche, and the first 40
genetic principal components, (2) standardised values were obtained by
inverse normal transforming the rank of the residuals.
