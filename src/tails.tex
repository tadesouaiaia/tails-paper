...
\label{fig:intro} \end{figure} \FloatBarrier 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%   END INTRO %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%   
%
...
\begin{itemize} 
\item \textit{Phosphate} - No POPout Effect in either tail, selected due to highest $h^2$ among such traits, 
...
\textbf{(a)} PRS quantile (in 1\% increments) plotted agaisnt mean trait value for four traits with
...
\subsection*{\emph{Replication and Validation}} ~\\

Shown in \fref{sup:fi0}, we performed four \emph{flavours} of replication and validation. 

\begin{enumerate}                                                                               
\item \textbf{Repeated Measures (averaged) on European Ancestry Individuals in the UKB:} Assess whether POPout signal results from random measurement error. 
\item \textbf{Non-European Ancestry Inviduals in UKB:}  Replicate the POPout results across ancestry (within biobank) 
\item \textbf{European Ancestry Individuals in All Of Us:} Replicate the POPout result across biobank (within ancestry) 
\item \textbf{Repeated Subsets of The Target Data:} Assess the reliability and power of the POPout test in smaller samples. 
\end{enumerate} 

In \fref{fig:rep}{a} trait quantile is plotted in 1\% increments versus mean PRS in the quantile of our index traits, with 
\textit{Height} and \textit{Weight} subsituting for \textit{Ankle Spacing Width} and \textit{Sitting Height} which are 
...
Finally, to put these numbers in perspective we perform bootstrap analyses using repeated sampling of 
our target population (\fref{fig:rep}{f}), demonstrating that we are not that far from the upper bound. 
...
POPout scores for which almost no rare variants of substantially large effect 
have been identified (see \fref{fig:prs}{e}).  This suggests that strong selection 
...
\caption{\footnotesize{{\bf Rare Variant and Selection Analsis}}} 
...
driveout POPout regression.  In \fref{fig:evo}{d-g} the results from our simulation are shown; stabilising 
...
After genetic QC, we initially selected all quantitve traits from the categories \textit{Biomarkers}, \textit{Physical Measures}, 
the \textit{Touchscreen Questionnaire} and \textit{Cognitive Tests} to provide a broad group of traits 
likely to have polygenic etiology.  We then removed 34,698 cancer cases, 1311 individuals on insulin, and 109 
pregnant women from all traits.  We also removed ~65k individuals on statins and other cholesterol lowering 
drugs from the \textit{Blood count} and \textit{Blood Biochemistry} subcategories and removed ~30k individuals on heart 
rate medication from the \textit{Arterial stiffness} and \textit{Blood pressure} subcategories.  Next, to ensure that 
extreme outliers do not impact our results, we removed all samples from traits more than six standard deviations 
from the mean.  After these sample removals, we defined quantitive traits as those which contained at least ten 
distinct values and whose top two modal values comprised less than half the samples. All such traits with at least 
$100,000$ total samples of european ancestry and at least $1000$ sibling pairs of european ancestry were then selected 
for the following stage of analysis. 

\paragraph*{Trait Normalisation} ~\\
To minimise the effect that environmental risk factors have on phenotype, trait values were then
normalised within each ancestry group by applying a linear regression model to each trait, with 
covariates for $age$, $age^2$, $age^3$, sex, menopause status, type-II diabetes status, coronary 
artery disease status, alcohol use (categorial and continuous), smoking (categorical and continuous), batch, centre, 
and the first 40 genetic principal components.  At this stage, traits whose residuals had absolute skew $>2$ in 
the larger European ancestry group were removed to avoid measurement bias and remaining trait values were 
standardized using a rank inverse normal transform on residuals to provide standard normal distributions 
for all further analysis. 


\paragraph*{Initial Analysis and Further Trait QC} ~\\
To carry out our initial trait analysis, European ancestry UKB samples were split 
randomly into 50\% base and 50\% target data sets. Then, for each trait, GWAS was 
carried out on the standaridised trait values in the base data using PLINK-1.9 \citep{plink, bt_10} 
and estimated heritability and pairwise genetic correlation was estimated using LDscore \cite{ldsc}.  
At this stage, to ensure that our PRS analyses are well powered and reflect additive polygenicity 
we removed traits with heritability $h^2<5\%$ or for which a single SNP explained more than 
2\% of trait variance.  This left 120 remaining quantitive traits from across all three categories. 


\paragraph*{Trait De-Duplication} ~\\
To ensure that the traits analysed were meaningly different from each other, we considered the 
...
geneticically non-unique traits came from two categories \textit{ Bone-densitometry of heel} %(18) 
...
duplicates and the 11 traits selected from duplicated pairs, this resulted in the set of 74 total traits which 
...
Approximately 20k European ancestry individuals in the UKB had measurements 
repeated after a five-year followup period.  These individuals had trait values normalised as 
above but were held out of the base dataset above.  Their residualised trait values from their 
two visits were then averaged to provide a target subset of individuals for which 
random measurement error is less likely to produce phenotype values in the tail.  
Replication analysis was restricted to the 63 of the 74 traits where there were at 
least 1000 repeated measures.



\paragraph*{Non-European Ancestry Individuals} ~\\
Approximately 22k Non-European ancestry individuals were normalised along with the 
European ancestry samples but also held out of the GWAS base dataset after trait normalisation.  
They were used to replicate the POPout results on the 70 of 74 traits where there were at least 
a thousand samples. 


\paragraph*{Replication in All of Us Cohort} ~\\
\textit{All of Us} is a multi-ancestry biobank from the United States 
that make use of electronic health record (EHR) data from a much wider age range than the UKB ($\mu,\sigma$= 55.8,17.1 vs 56.5,8.1). 
Traits matching those analysed in our primary UKB analysis 
were identified through an initial database search and were then interrogated 
manually to determine if trait descriptions were sufficiently similar.  
All of Us provides ancestry categories, corresponding to definitions used within gnomAD, the Human
Genome Diversity Project, and 1000 Genomes project.  133,581  European ancestry individuals were 
extracted for analysis. %Too few individuals of non-European ancestry were avaialble for sufficient replication power.
...
allele frequency $<$ 0.01, genotype missingness > 0.02, or Hardy-Weinberg equilibrium test $P$-value $< 10^{-8}$ were
excluded. Samples exhibiting high levels of missingness or heterozygosity, inconsistencies in genetic-inferred 
and self-reported sex, or displaying aneuploidy of the sex chromosomes were removed and the same 
greedy algorithm mentioned above was used to maximize sample retention while removing all third-degree relatives (kinship
coefficient > 0.044) \citep{erase}.  Using the higher powered publicly available European ancestry GWAS summary stats 
generated from the entire UKB \cite{neale}, PRS scores were calculated \cite{prsice} using the same p-value 
threshold that was trained in the UKB European ancestry analysis described above. After removing traits 
for which fewer than 1000 samples remained this left us witih 33 trait phenotype and PRS pairs for replication 
in All Of Us. 



\paragraph*{Bootstrap Analysis using European Ancestry Target Subsets} ~\\
Our 74 QC-validated traits were required to have at least 100k total
samples.  In the target sample (half the total), after outlier removal
our QC-validated traits had between of 50,109 and 167,857 (average
123k) in the target sample.  To explore whether our PRM-test results
...
\subsubsection*{POPout regression test QC} ~\\
...
\subsubsection*{POPout Rare Variant Associations} ~\\
...
\subsubsection*{Association between POPout Test and LRS Models} ~\\ 
% NO THIS IS WRONG -- WILL FIX THIS TADE 
...
\textbf{(4)} miscarriages and still-births (Trait-Ids: 39290 and 3839), 
and \textbf{(5)} paternal age which is calculated as the difference 
between fathers age (Trait-Id 2946) and participant age at recruitment (Trait-Id 21022).
...
follows.  The mean value of the five proxies of selection was
...
proxy and the $POPout$ effect size.
...
\subsubsection*{Pseudo Case Control Analyses}~\\ 

The predictive value of PRS in the tails of the distribution 
can be calculated by setting a tail threshold and calculating the 
odds-ratio under a pseudo case-control analysis.  Here we separate 
samples accoriding to PRS quantiles that include the lowest 5\%, between 
5-15\%, 15-25\%, 25-35\%, 45-55\%,55-65\%, 65-75\%, 75-85\%, 85-95\% and 
the top 95\%.  Then for each trait threshold k (where case status is 
defined as being more extreme than k), the odds ratio and 95\% confidence 
interval is calculated for each quantile by comparing it to the 45-55\% quantile. 
...
\FloatBarrier \begin{figure}[!h] \begin{maybe}{showFig} 
\includegraphics[trim=0cm 0cm 0cm 0cm, clip, width=\linewidth]{Figures/fig5.pdf} \end{maybe} 
\caption{\footnotesize{{\bf POPout test replication and validation results in UKB repeated measures, UKB non-European samples, All of Us
European ancestry.} 
...
samples. \textbf{(f)}....}  } \label{fig:sup0} \end{figure}
\FloatBarrier
