...
For the sibling analyses, sibling pairs of European ancestry were
selected by first identifying first degree relatives defined as those
with kinship coefficient within the range 0.177 and 0.354, this range
on either side of the expected value of 0.25 was used to account for
the expected variation in within-pair similarity \citep{sib1}. The
proportion of SNPs with zero identity-by-state (IBS0) was used to
distinguish sibling pairs from parent-offspring pairs since
parent–offspring pairs have IBS0$\approx$0. It has been shown in the
UKB that there is good separation between pairs of first degree
relatives at IBS0=0.00112 \citep{sib1}, therefore, sibling pairs were
selected as those exceeding this threshold, this gave a total of
17,289.
...
Quantitative traits were selected for analysis, we defined
quantitative traits as those having as at least 10 distinct values and
top two modal trait values having less than half of the samples. To
ensure our analyses were well powered, traits with heritability
$h^2<5\%$ (measured by LD score \citep{ldsc} applied to publicly
available UKB GWAS summary statistics \citep{neale}) and $<$50,000
observation in unrelated samples were removed. Trait specific QC first
first removed traits values more than six standard deviations from the
mean. Trait values were then standardised by applying linear
regression model to each trait, with covariates for age, sex,
recruitment centre, genotyping batch, and the first 40 genetic
principal components.  Subsequently, traits with skewed ($>2$) or
multimodal residuals were removed (dip-test \citep{dip} P-value for
unimodality $>$ 0.05). Standardised values were obtained by inverse
normal transforming the rank of the residuals. To ensure selected
traits were polygenic traits which had a single variant $r^2 > 0.5\%$
were removed.
