...
primary dataset of unrelated individuals of European ancestry which result in a set of 180 traits 
which were then further normalised as described below. 
...
type-II diabetes status, coronary artery disease status, 
alcohol intake (Field ID: 1558), 
...
was suggested due to a single SNP explained more than 2\% of trait variance \cite{2prule}. 
This left 120 remaining quantitative traits from across all four categories. 

To ensure that the traits were meaningfully different from each other, we considered the 
...
applied to the target dataset to produce risk scores. 
...
2461 and 15475 (average: 10699.4) unrelated individuals for repeated measure replication. 
PRSice was used to generate PRS scores using the threshold from the European dataset 
described above. 
...
is a separate run that included all ancestry samples.   Across the 74 traits there 
were between 569 and 22513 (average: 14871.4) unrelated individuals for cross ancestry replication. 
