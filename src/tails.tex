...
To quantify this we develop a statistical test that estimates effect
...
distribution. Specifically, a regression of PRS on standardised trait
values is fit. The regression effect size in the tail is defined as
the mean of the residuals in the lower tail and minus the mean of the
residuals in the upper tail. Using this definition, positive values
in both the upper and lower tails correspond to regression to the
mean, whereas negative values correspond to regression away from the
mean.
...
The test is applied to 75 continuous UKB traits with heritabilty
$>0.05$, PRS $R^2>0.01$, sample size $>$ 10,000 and thinned such that
the maximum pairwise trait correlation $<$, see Methods for further
details of trait QC. Using these 75 traits we perform PRM-Testing,
rare-variant analysis, analysis of selection to help elucidate the
mechanism that may be behind PRM.  We then replicate these results
using a sibling model and different datasets.
