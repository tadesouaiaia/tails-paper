...
22k\need{exact number} samples that represented multiple non-European
ancestries.
...
have IBS0$\approx$0. It has been shown in the UKB that there 
is good separation between pairs of first degree relatives at
IBS0=0.00112 \citep{sib1}, therefore, sibling pairs were selected as those
...
analysis).\need{not clear from this if all sib pairs or just EUR sib
  pairs were used?}
...
\clearpage 
\newpage
After genetic QC, cancer patients, etc were removed.....  Quantitative
traits were selected for analysis, defined as at least 10 distinct
values and top two modal trait values include less than half of the
samples. To ensure our analyses were well powered, traits with
heritability $h^2<5\%$ (measured by LD score \citep{ldsc} applied to
publicly available UKB GWAS summary statistics \citep{neale}) and
$<$50,000 observation in unrelated samples were removed. Traits values
more than six standard deviations from the mean were removed. Traits
values were then standardised as follows: (1) linear regression models
were applied to each trait, with covariates for age, sex, recruitment
centre, genotyping batche, and the first 40 genetic principal
components.  Subsequently, traits with skewed ($>2$) or multimodal
residuals were removed (dip-test\citep{dip} P-value for unimodality
$>$ 0.05). Standardised values were obtained by inverse normal
transforming the rank of the residuals. Furthermore, we selected
polygenic traits by removing traits which had a single variant $r^2 >
0.5\%$, leaving 123 traits.

To ensure the traits analysed were unique, pairwise genetic
correlation was calculated using LDscore\cite{ldsc}. Sixty one traits
did not have a pair with $r_g>98\%$ and were kept for analysis.
Additionally, 10 traits has a single almost identical match (eg:
"Standing Height" (ID:50) and "Height Measured Prior to Imaging"
(ID:12144)), in these cases the trait with greater sample size was
priotised.  The other 52 traits were part of larger clusters of
...
data maximization algorithm to select unrelated individuals
\citep{erase}, 11 traits were selected resulting in a set of 77
genetically unique traits for analysis.
...
process yielded 32 overlapping traits, 25 of which passed our
PRM-QC. These traits have between 8595 and 127677 observations
(average 73k) in the European ancestry samples. \need{ancestry of
  samples used in }
