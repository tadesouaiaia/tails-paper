...
The primary goal of this study is to identify and characterize
\emph{polygenic-regression to the mean} (PRM), which occurs when
individuals in the tails of the quantitive trait distribution exhibit
polygenic risk that is closer to the mean risk score than expected.
To quantify this phenomenon we developed a quantitive trait
statistical test (PRM-test - see methods) that estimates effect size,
direction, and signficance of movement away from the expected
polygenic load for individuals in the lower and upper 1\% of the
population.  Applying this test to the UKB reveals that 400 of 1,185
heritiable ($h^2>0.01$) quantitive traits have signficant ($p<0.05$)
movement from expectation in one or both tails and that 95\% of the
time this movement is toward the mean (see - Supplement).  Observing
that many of these tests are underpowered due to insufficient sample
size and poor polygenic prediction ($R^2$), we identify a set of 75
...
\caption{\footnotesize{ \textbf{(a)} Four traits the demonstrate PRS
    tail regression (clockwise: No Tail Regression, Upper Tail, Lower
    Tail, Dual Tail).  \textbf{(b)} Miami plots demonstrates that dual
    tail regression is dominant tail regression observed.
    \textbf{(c)} Trumpet plot reveal highly penetrant rare variants in
    many of the trait tails where PRM is obersved.  \textbf{(d)} Tail
    regression is associated with greater a number of penentrant rare
    variants.  \textbf{(e)} Age of Menopause is a counterexample.
