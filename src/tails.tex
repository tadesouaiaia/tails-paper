...
quantitative traits as those having at least 10 distinct values and
for which the top two modal trait values combined comprised fewer than
half of the samples. To ensure that our analyses were well-powered,
traits with heritability $h^2<5\%$ (estimated by LD score \citep{ldsc}
applied to publicly available UKB GWAS summary statistics
\citep{neale}) and $<$50,000 unrelated samples with observed trait
measures were removed. Trait specific QC was then performed to first
remove trait values more than six standard deviations from the mean,
so that extreme outliers did not impact results. TO MINIMISE THE
EFFECT OF ENVIRONMENTAL RISK FACTORS ON THE RESULTS??, trait values
were then standardised by applying a linear regression model to each
trait, with covariates for age, sex, recruitment centre, genotyping
batch, and the first 40 genetic principal components [WHAT ABOUT AGE2
ETC AND THE FULL REGRESSION THAT WAS PERFORMED??].  Subsequently,
traits with skewed ($>2$) or multimodal residuals were removed
(dip-test \citep{dip} $P$-value for unimodality $>$
...
\noindent All Of Us is a biobank containing XX binary and XX
continuous traits from individuals of multiple ancestries from across
the United States of America. Traits matching those from the 75 traits
of the primary analyses (performed in the UK) were identified through
an initial database search and were then interrogated manually to
determine if trait descriptions were sufficiently
similar. \need{European ancestry individuals were extract by....}.
\need{Trait values were normalised by...}  There are too few
...
parameters with $\gamma<0$ stabilising selection was inferred, but the
population trait mean is moving towards an optimal value. With
