...
those which contained at least ten distinct values and whose top two modal values comprised less than 
...
% ADD IN INFOR FOR PRSICE HERE - FASTSCORE, hi-res PRM 
% ADD IN CORRELATION BETWEEN PAIRWISE CORRELATION AND STANDARDIZED DIFFERENCE OF POPOUT IN TAIL % 
...
2461 and 15,475 (average: 10699.4) unrelated individuals for repeated measure replication. 
PRSice \cite{prsice} was used to generate PRS scores using the threshold from the 
unrelated European dataset described above. 
...
%repeated after a five-year followup period.  
%These individuals had trait values normalised as 
%above but were held out of the base dataset above.  Their 
%residualised trait values from their 
...
%least 1000 repeated measures.  % WHY A THOUSAND AND HOW DO WE KNOW ITS POWERFUL ENOUGH 
...
threshold that was trained in the UKB European ancestry analysis described above. 

%After removing traits 
...
performed 100 trials across each of the 148 trait tails and calculated the linear and 
the rank correlation for $POPout$ effect size across the tails.  We also enumerated 
the number of of $POPout$ effects that were within the larger dataset 95\% confidence interval, 
and the number of traits for which the greater tail $POPOut$ effect matched the larger dataset. 
(see \textit{Supplement}). 
...
In each dataset matching traits with at least five thousand samples that passed the 
...
\caption{\footnotesize{\textbf{TraitTables}}} \label{fig:sup1} \end{figure} \FloatBarrier


\clearpage \newpage
\FloatBarrier \begin{figure}[!h] \begin{maybe}{showFig} 
\includegraphics[trim=0cm 0cm 0cm 0cm, clip, width=\linewidth]{Figures/Sup2.pdf} \end{maybe}
\caption{\footnotesize{\textbf{BootStrap Analysis}}} \label{fig:sup2} \end{figure} \FloatBarrier

\clearpage \newpage
\FloatBarrier \begin{figure}[!h] \begin{maybe}{showFig} 
\includegraphics[trim=0cm 0cm 0cm 0cm, clip, width=\linewidth]{Figures/Sup3.pdf} \end{maybe}
\caption{\footnotesize{\textbf{Extended Replication}}} \label{fig:sup3} \end{figure} \FloatBarrier
