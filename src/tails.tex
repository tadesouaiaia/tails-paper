...
genotype QC. Unimodal, non-skewed, continuous traits with heritabilty
$>0.05$, PRS $R^2>0.01$ and sample size $>$50,000 were selected for
analysis. After removal of strongly correlated traits (maximum
pairwise correlation $R^2<$0.95 ) 77 traits remained for
analysis. Traits were standardised, removing effects of age, sex,
recruitment centre, genotyping batch, and principal components and
transformed to be normally distributed, see Methods for further
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%   RESULTS-1  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\dref{fig:prs}{a} shows 

the most significant trait examples of how
tail regression is classified: no tails regression exhibited by
triglycerides (chosen as the trait with highest $h^2$ of any trait
without tail regression), only lower tail regression exhibited by
haemoglobin concentration, only upper tail exhibited by red blood cell
distribution width and dual tail exhibited by sitting height.

Running our PRS-test on all 75 traits reveals that the 80\% of tails
(121 of 150, FDR level 0.05) exhibit significant (FDR$<$0.05) movement
away from expectation and that 98\% (118 of 121) of the time the
movement that occurs is regression toward the mean. \dref{fig:prs}{b})
shows results for all traits with significant movement towards the mean.

The majority of
traits (47 of 75) were classified as "Dual Tail" (significant
regression to the mean in both tails), while 15 were classified as
"Lower Tail" and 9 were classified as "Upper Tail" (see
\dref{fig:prs}{b}). For illustrative purposes we identified four
"index traits" to represent "No Regression" (Triglycerides - the
highest $h^2$ of any trait without tail regression), "Lower Tail"
(Haemoglobin Concentration - most signficant), "Upper Tail" (RBC
Distribution Width - most significant), and "Dual Tail" (Sitting
Height - most significant maximum pvalue).  The tail dynamics observed
in each of these traits can be illustrated in \dref{fig:prs}{a}.
