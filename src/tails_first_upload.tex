...
The traits analyzed in the following section were arrived at by first considering 
all continuous (at least 10 unique values) traits in the UK Biobank from three major categories: 
\textit{Biomarkers}, \textit{Physical Measures} and the \textit{Touchscreen Questionnaire \& Cognitive Tests}.  
Initially, using only UKB samples of European ancestry, traits were standardised by regressing out the 
effects of age, sex, and known environmental and technical factors (see \textit{Methods1}).  Then samples 
were split in half and GWAS analysis was carried out using plink \cite{plink} and heritability and pairwise genetic correlation was 
estimated using LDscore \cite{ldsc}.  Applying quality controls for sample size, heritability, 
skew, and removing strongly correlated traits left 74 four traits for analysis (see \textit{Methods2}), which 
were analyzed for POPout and associated analyses across large number of European ancestry samples in the UK Biobank. 
This result was replicated across the subset of repeated measures European ancestry samples in the UK Biobank, 
across the subset of non-European ancestry samples in the UK Biobank, and, where applicable across European ancestry 
samples in the All Of Us Biobank (see \textit{Replication}).  
...
The POPout tests identifies traits where the samples in one or both tails 
(the lower and upper 1\% of the distribution) have expectedly large or 
small average PRS values.   Applying this test to the European ancestry 
samples in the UK Biobank reveals widespread POPout effect



Applying the POPout test to 74 Traits to the  European ancestry samples in the UK Biobank reveals that 
...
%for GWAS and subsequently estimate PRS via PRSice and our PRS-test was
%applied to the remaining 50\% of samples. Throughout, PRS were
%estimated with variants with MAF$>$1\%, see Methods for further
%details of genotype quality control. Unimodal, non-skewed, continuous
%traits with heritabilty $>0.05$, PRS $R^2>0.01$ and sample size
%$>$50,000 were selected for analysis. Strongly correlated traits
%(maximum pairwise correlation $R^2<$0.95) were identified and the
%trait with the smaller sample size was removed, leaving 77 traits for
%analysis. 


%Traits were standardised by regressing out the effects of
%age, sex, recruitment centre, genotyping batch, and the top 40
%principal components in a linear regression model and quantile normal
%transforming the residuals to give values for analysis, see Methods
%for further details of trait QC and selection. We then applied a
%quality control metric specific to our test that tests for significant
%departure from predicted values from the regression in the middle 90\%
%or the trait distribution (see Methods). Two traits failed this test
%leaving 75 traits (150 tails) for analysis. Extended Data Table 1
%lists all 77 traits and their available sanple sizes.
...
Typically, variants included in PRS have minor allele frequency
(MAF)$>$1\%.  The exclusion of rare variants from PRS has been shown
to reduce PRS performance \citep{rares1}. \fref{fig:prs}{c} shows
Trumpet plots, MAF vrersus effect size, for genome-wide significant
associations from GWAS \citep{neale} and exome analyses
\citep{backman} of UKB for our four foucus traits. For Haemoglobin
Concentration which exhibits lower tail regression we see a greater
number of rare variants in the lower tail and for Red Blood Cell
...
where the PRM Effect is greatest. For ankle spacing width which does
not exhibit PRM in either tail there are just three significant rare
effect variants.  This result is consistent with the hypothesis that
rare variants of large effect that not accounted for in the
common-variant PRSare responsible for the tail specific PRM.
...
variants in the tail across all 150 trait tails analysed. However, for
some traits the tail with more PRM shows almost no rare variants (see
\fref{fig:prs}{e}) suggesting that strong selection may result in
private variants that cannot be identified using population based
sequencing.
...
\caption{\footnotesize{{\bf Selection analyses in real and simulated
    ...
    Best model fit to the data, of all models with and without linear
    and quadratic terms, selected by BIC, are shown by dashed
    ...
    fitness measure correlation was calculated between the mean of
    the fitness measure in individuals in the upper and lower 1\%
    tails and PRS-test effect size for the corresponding tail across
    74 traits in UKB passing QC. Fitness measures were standardised
...
\FloatBarrier \begin{figure}[!h] 
  ...
  \caption{\footnotesize{{\bf Sibling analyses for rare variants of
      ...
      Mendelian. \textbf{(b)} {\it LDscore regression} SNP
