...
general (\dref{fig2}{b}, Supplementary Table 1). Of the 107 tails with significant 
...
the UK Biobank Resource under application 18177 (Dr O’Reilly). All participants gave full informed consent. This work was supported through the computational resources and staff expertise provided by the Data Ark and Scientific Computing teams at
the Icahn School of Medicine at Mount Sinai. We a grateful for helpful discussions about this work with: Alanna Cote, Carla Giner-Delgado, Conrad Iyegbe, Judit García-González, Lathan Liou, Laura Sloofman and Louise Wain. 
...
were sufficiently similar (see Supplementary Table 1).  
...
Genetic QC in All of Us was conducted using standard filters, as used in the UKB.  SNPs with MAF $<$ 0.01, genotype missingness > 0.02, or Hardy-Weinberg equilibrium test $P < 10^{-8}$ were excluded. The same greedy algorithm used to maximise sample 
...
population of N=10,000 diploid individuals of genome size100kb was simulated to
...
purposes, "weak selection" corresponded to simulations performed using a multiplier of $k=1$ in equation 3, while "strong selection" corresponded to simulations with $k=100$. "Large effects" were considered those in the top 10\% of effect sizes genome-wide.   
The mean of the log-transformed odds ratio across all 100 independent simulation replicates was used as a measure 
of enrichment and a $t$-test was used to test for significant differences from zero (see \dref{fig4}{d}).  
To analyse the impact that low-frequency variants have on prediction and to mimic the calculation of common 
variant PRS in real data, a second genetic value was calculated using only variants with \emph{MAF} $>1\%$ 
and compared to the true trait value across the individuals in each centile.  The mean difference (POPout) 
and 95\% confidence intervals in each centile were calculated across the 100 independent simulation replicates (\dref{fig4}{e}).
